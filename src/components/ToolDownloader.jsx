...
const ToolDownloader = () => {
  ...
  const handleDownloadClick = async () => {
    if (inputValue) {
      ...
      try {
        const response = await submit(inputValue);
        setStatus("success");
  ...
  const submit = async (x) => {
    ...
    const url = encodeURIComponent(cURL);
    ...
    const options = {
      method: "GET",
      url: "https://instagram-downloader19.p.rapidapi.com/index",
      params: {
        url: "https://www.instagram.com/p/Bhr1Ca9lcT0/",
      },
      headers: {
        "X-RapidAPI-Key": "3c9f0d7da5mshef3cf3b9c8fc152p175479jsne7b29582c3f3",
        "X-RapidAPI-Host": "instagram-downloader19.p.rapidapi.com",
    ...
    await axios
      ...
      .then((response) => {
        ...
        setdownloadedData(response.data.items);
      })
      .catch((error) => {
  ...
  return (
    <>
      ...
      <section className="container mx-auto text-center">
        {" "}
        <div
          className="w-full bg-white rounded-md p-6 flex flex-col items-center"
          style={{ gap: 14 }}
        >
          {" "}
          {status === "loading" && (
